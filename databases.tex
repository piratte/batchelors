...
then the production data would have, but gave the testing more data to be based upon.
...
\subsection{Testing query performance on a query sample}
...
indexed. Indexes were made on several integer fields (testInt1-8) and three string fields 
(testStr1,testStr2,testStr3). The time cost of building the indexes and their space 
requirements\footnote{for the default storage engine} are listed in table \ref{tab:indexBuildTimes}.

\begin{table}[h]
\centering
\caption{The time and storage requirements for indexes used in MongoDB}
\label{tab:indexBuildTimes}
\begin{tabular}{|l|l|l|l|l|}
\hline
Field    & Time     & Space (default) & Space (VT with zlib) & Space (VT with zlib) \\ \hline
testInt1 & 00:20:50 & not measured    &                      &                      \\ \hline
testInt2 & 00:20:00 & not measured    &                      &                      \\ \hline
testInt3 & 00:19:59 & 169936          &                      &                      \\ \hline
testInt4 & 00:20:12 & 169972          &                      &                      \\ \hline
testInt5 & 00:19:58 & 169944          &                      &                      \\ \hline
testStr1 & 00:20:06 &                 &                      &                      \\ \hline
testStr2 & 00:20:13 &                 &                      &                      \\ \hline
testStr3 & 00:20:51 &                 &                      &                      \\ \hline
testInt6 & 00:21:03 &                 &                      &                      \\ \hline
testInt7 & 00:19:57 &                 &                      &                      \\ \hline
testInt8 & 00:19:58 &                 &                      &                      \\ \hline
\end{tabular}
\end{table}
...
The queries were made randomly combining the integer and string properties. The number of hits was not considered
when generating the queries. All the queries used are 
%TODO add queries to an appendix?
...
The original MongoDB instance was using the old storage engine used by versions up to 3.0\footnote{MMAPv1 Storage 
Engine based on memory mapped files}. Since then new storage engine WiredTiger\footnote{
\url{https://docs.mongodb.org/manual/core/wiredtiger/}} became available so the performances could compared.
...
\begin{table}[]
\centering
\caption{Disk usage comparison of MongoDBs storage engines}
\label{tab:MongoComp}
\begin{tabular}{|l|r|r|}
\hline
Storage engine                      & \multicolumn{1}{l|}{Total database size (MB)} & \multicolumn{1}{l|}{Total index size} \\ \hline
MMAPv1                              & 136 161                                       &                                       \\ \hline
WiredTiger with zlib compression    & 22 788                                        &                                       \\ \hline
WiredTiger with default compression & 42 249                                        & 169936                                \\ \hline
\end{tabular}
\end{table}
